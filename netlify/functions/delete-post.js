...
function canDeletePost(user, frontmatter) {
  if (user.role === "admin") {
    return true;
  }

  const expectedAuthor = ROLE_OWNERS[user.role];
...
export async function handler(event) {
  ...
  try {
    ...
    /*
     * Se obtiene primero el archivo para:
     *
     * 1. recuperar el SHA solicitado por GitHub;
     * 2. leer el autor almacenado;
     * 3. comprobar que el usuario pueda eliminarlo.
     */
    ...
    if (!sha) {
      return jsonResponse(500, {
        error:
          "GitHub no devolvió el SHA requerido para eliminar el artículo.",
      });
    }
    ...
    if (
      ...
      return jsonResponse(500, {
        error:
          "No fue posible leer los datos editoriales del artículo.",
      });
    }

    if (!canDeletePost(user, frontmatter)) {
      return jsonResponse(403, {
        error:
          "No tienes permiso para eliminar este artículo porque pertenece a otra dirección.",
      });
    }
    ...
    return jsonResponse(500, {
      error:
        error instanceof Error
          ? error.message
          : "Ocurrió un error inesperado al eliminar el artículo.",
    });
  }
}